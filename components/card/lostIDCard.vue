<template>
	<view>
		...
		<form class="corMessage" report-submit @submit="formSubmit">
			<view>
				身份证号:<input type="idcard" v-model="IDnum"/>
				姓名:<input type="text" v-model="IDname"/>
			</view>
			...
			<view class="cor_input">
				...
				<input v-if="relationType==='qq'" type="text"
					placeholder="填写qq"  v-model="relation">
				<input v-if="relationType==='tel'" type="text"
				...
				uni.chooseImage({
					...
					success:(res)=>{
						...
						this.$api.pubCor(that.photo[0],{
							...
							this.IDnum=data['公民身份号码'].words;
							this.IDname=data['姓名'].words;
							that.isScan=false;
						})
					}
				})
